...
pid = PIDController(kp=kp, ki=ki, kd=kd, max_windup=200,
                    u_bounds=[-umax, umax], alpha=alpha)
...
def pointFromBezier(points, t):
    point = []
    point[0] = ((1-t) ** 3)*p[0][0] + ((1-t)**2)*3*t * \
        p[1][0] + (1-t)*3*t*t*p[2][0] + (t**3)*p[3][0]

    point[1] = ((1-t) ** 3)*p[0][1] + ((1-t)**2)*3*t * \
        p[1][1] + (1-t)*3*t*t*p[2][1] + (t**3)*p[3][1]

    return point
    

def followBezier(points):
    points = [tuple(map(int, p.split(','))) for p in points]
    locations = []
    for t in range(0, 1, 1/10):
        locations.append(pointFromBezier(points, t))
    locations = [pixelToMmList([x,y]) for [x,y] in locations]
    print(locations)
    while True:
        ...
        centersMM = pixelToMm(
            (float(centers[0][0]), float(centers[0][1])), w1, h1)
        if frames == 100:
            (x0, y0) = centersMM
        angle = angles[0][0]
